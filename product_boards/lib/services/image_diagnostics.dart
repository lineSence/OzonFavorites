...
class ImageDiagnostics {
  ...
  static void start(String url, {String? referer}) {
    log('START', {'url': url, 'referer': referer});
  }

  static void candidate(String source, String? url) {
    log('CANDIDATE', {'source': source, 'url': url});
  }
  ...
  static void result(String message, {String? url, String? path}) {
    log(message, {'url': url, 'path': path});
  }

  static void failure(String stage, Object error, {String? url, StackTrace? stackTrace}) {
    log('FAIL_$stage', {'url': url, 'error': error});
    if (stackTrace != null) {
      developer.log(stackTrace.toString(), name: _name, level: 900);
    }
  }
}
