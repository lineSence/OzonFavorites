...
class _SmartSortPageState extends State<SmartSortPage> {
  ...
  Map<String, List<SmartSortResult>> get _groups {
    ...
    for (final result in _results) groups.putIfAbsent(result.category, () => []).add(result);
    return groups;
  }
  ...
  Future<void> _apply() async {
    ...
    try {
      final byName = <String, Category>{for (final category in _categories) category.name.trim().toLowerCase(): category};
      for (final group in confident.map((r) => r.category).toSet()) {
        if (byName.containsKey(group.toLowerCase())) continue;
        final now = DateTime.now();
        final category = Category(id: widget.repository.newId(), name: group, createdAt: now, updatedAt: now);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [if (_results.isNotEmpty) IconButton(tooltip: 'Повторить анализ', onPressed: () => setState(() => _results = _service.classifyAll(widget.items)), icon: const Icon(Icons.refresh))],
      ),
      body: widget.items.isEmpty
          ...
          : ListView(
              ...
              children: [
                const Text('Локальная умная сортировка', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
                const SizedBox(height: 6),
                Text('Анализируются только название, заметка и ссылка. Данные никуда не отправляются.', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)),
      ...
      bottomNavigationBar: SafeArea(
        ...
        child: FilledButton.icon(
          ...
          icon: _applying ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.auto_awesome),
    ...
    return Card(
      ...
      child: ExpansionTile(
        ...
        children: results.map((result) => ListTile(
          ...
          subtitle: Text(result.matchedKeywords.isEmpty ? 'Нет совпадений — останется на месте' : 'Совпадение: ${result.matchedKeywords.join(', ')} · ${(result.score * 100).round()}%'),
