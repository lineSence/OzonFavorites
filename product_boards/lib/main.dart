...
class _ArchiveScreenState extends State<ArchiveScreen> {
  ...
  Future<bool> _confirmDuplicate(List<ArchiveItem> duplicates) async {
    final locations = duplicates.map((item) {
      final category = item.categoryId == null ? null : _categories.firstWhereOrNull((c) => c.id == item.categoryId);
  ...
  Future<void> _openDetail(ArchiveItem item) async {
    final result = await Navigator.push<ArchiveItem>(
      context,
      MaterialPageRoute(builder: (_) => ArchiveItemPage(repository: widget.repository, queue: widget.queue, item: item, categories: _categories)),
    );
    if (!mounted) return;
    if (result != null) {
      await widget.repository.upsertItem(result);
  ...
  String _plural(int n, {bool deleteForm = false}) {
    final x = n.abs() % 100;
    if (x >= 11 && x <= 14) return deleteForm ? 'ссылок' : 'ссылок';
    switch (x % 10) {
      ...
      appBar: AppBar(
        ...
        title: _selectionMode
            ...
            : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Мой архив', style: TextStyle(fontWeight: FontWeight.w900)),
                      ...
                      child: GridView.builder(
                        ...
                        itemBuilder: (_, index) {
                          ...
                          return ArchiveCard(
                            ...
                            onTap: () => _selectionMode ? _select(item.id) : _openDetail(item),
          ...
          child: ListView(
            ...
            children: [
              ...
              ListTile(
                ...
                title: const Text('Неразобранное'),
                selected: _categoryId == null,
                onTap: () { Navigator.pop(context); setState(() => _categoryId = null); unawaited(_reload()); },
              ),
              const Divider(),
              ..._categories.map((category) => ListTile(
                    ...
                    trailing: PopupMenuButton<String>(
                      onSelected: (value) {
                        if (value == 'rename') unawaited(_renameCategory(category));
                        if (value == 'delete') unawaited(_deleteCategory(category));
                      },
                      itemBuilder: (_) => const [
  ...
  Widget _emptyBody(String title) => Center(
        child: Padding(
  ...
  Widget _errorBody() => Center(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(height: 12),
            const Text('Не удалось загрузить архив', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900)),
            const SizedBox(height: 8),
            Text(_loadError ?? '', textAlign: TextAlign.center, maxLines: 5, overflow: TextOverflow.ellipsis),
            const SizedBox(height: 16),
            FilledButton.icon(onPressed: _reload, icon: const Icon(Icons.refresh), label: const Text('Повторить')),
          ]),
        ),
      );
}
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 18),
            ListTile(contentPadding: EdgeInsets.zero, leading: const Icon(Icons.folder_outlined), title: Text(item.categoryId == null ? 'Неразобранное' : categories.firstWhereOrNull((c) => c.id == item.categoryId)?.name ?? 'Подборка')),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ClipRRect(borderRadius: BorderRadius.circular(18), child: _image(widget.item.imageUrl, 240)),
        const SizedBox(height: 18),
